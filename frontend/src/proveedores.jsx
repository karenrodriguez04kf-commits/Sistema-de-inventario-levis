...
const proveedores = () => {
    ...
    return (
        <div className="proveedores-container">
            <h2 className="proveedores-titulo">Administración de Proveedores</h2>
            <form onSubmit={handleSubmit} className="proveedores-form">
                <input type="text" placeholder="NOMBRE" value={nombre} onChange={e => setNombre(e.target.value)} required />
                <input type="email" placeholder="CORREO" value={correo} onChange={e => setCorreo(e.target.value)} required />
                <input type="text" placeholder="ROL DEL PROVEEDOR" value={rol_proveedor} onChange={e => setRolProveedor(e.target.value)} required />
                ...
                <tbody>
                    {proveedores.map(proveedor => (
                        <tr key={proveedor.id}>
                            ...
                            <td>
                                <button onClick={() => iniciarEdicion(proveedor)} className="btn btn-editar">Editar</button>
                                <button onClick={() => eliminarProveedor(proveedor.id)} className="btn btn-eliminar">Eliminar</button>
                            </td>
                        </tr>
                    ))}
                </tbody>

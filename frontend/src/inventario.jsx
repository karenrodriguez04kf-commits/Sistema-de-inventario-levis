...
function Inventario() {
  ...
  const [idProveedor, setIdProveedor] = useState(""); // Estado para el proveedor seleccionado
  const [idTalla, setIdTalla] = useState(""); // Estado para la talla seleccionada
  const [idGenero, setIdGenero] = useState(""); // Estado para el género seleccionado
  const [idCategoria, setIdCategoria] = useState("pantalon"); // Estado para la categoría seleccionada
  ...
  const guardarProducto = async () => {
    ...
    if (!form.id_talla) return alert("La talla es obligatoria");
    if (!form.id_genero) return alert(" El género es obligatorio");
    if (!form.id_categoria) return alert("La categoría es obligatoria");
    ...
    data.append("id_talla", form.id_talla);
